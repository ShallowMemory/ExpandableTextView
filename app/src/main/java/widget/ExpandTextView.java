...
import com.igeek.expandtextview.R;
...
public class ExpandTextView extends View implements View.OnClickListener {
    ...
    //收缩收起时候的提示图标
    private String expandDrawable;
    //展开时候的提示图标
    private String shrinkDrawable;
    //提示图标的宽度
    ...
    public ExpandTextView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        textColorMore = ta.getColor(R.styleable.ExpandTextView_moreTextColor, 14);
        expandDrawable = ta.getString(R.styleable.ExpandTextView_expandDrawable);
        shrinkDrawable = ta.getString(R.styleable.ExpandTextView_shrinkDrawable);
        ...
        moreTextPaint = new TextPaint(Paint.ANTI_ALIAS_FLAG);
        moreTextPaint.setColor(textColorMore);

        initPaint(textColor, textPaint, context);
        initPaint(textColorMore, moreTextPaint, context);

        drawableWidth = moreTextPaint.measureText("【展开】");
    }

    private void initPaint(int textColor, TextPaint paint, Context context){
        ...
        for (int index = 0; index < targetLine; index++) {
            ...
            if (index < targetLine - 1) {
                ...
                if (targetLine == maxLines && maxLines < lineTexts.size()) {
                    ...
                    if (showTipDrawalbe){
                        float left=getWidth() - drawableWidth - getPaddingRight();
                        canvas.drawText(shrinkDrawable, left, lineText.getBaseLine(), moreTextPaint);
                    }
                    ...
                    if (showTipDrawalbe){
                        float left=getWidth() - drawableWidth - getPaddingRight();
                        canvas.drawText(expandDrawable, left, lineText.getBaseLine(), moreTextPaint);
                    }
                }
            }
        }
